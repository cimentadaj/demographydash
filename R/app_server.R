...
observeEvent(input$nav_forecast, {
    ...
    if (current_tab() == "pop_page") {
      rintrojs::introjs(session, options = list(
        steps = data.frame(
          ...
          intro = c(
            i18n$translate("Customize the starting population if you wish—otherwise keep the UN defaults and continue."),
            i18n$translate("Review the baseline population numbers. They update automatically when you customise."),
            i18n$translate("Use the sidebar to jump to Fertility, Life Expectancy, Migration, or Run Projection. Every section is optional.")
          ),
  ...
  observeEvent(input$forecast_help, {
    if (current_tab() == "forecast_page") {
      rintrojs::introjs(session, options = list(
        steps = data.frame(
          element = c("#select_id", "#show_forecast_results_ui", "#left_menu"),
          intro = c(
            i18n$translate("Choose which projection story to view—population structure, fertility, migration, and more."),
            i18n$translate("Explore the charts and downloads generated by the last Run Projection. They refresh whenever you rerun."),
            i18n$translate("Need changes? Use the sidebar to revisit inputs or click Run Projection again from the Migration section.")
          ),
          stringsAsFactors = FALSE
        )
      ))
